...
class AuthController {
  static getUser = async (req, res) => {
    ...
    const decrypted_external_user_id = decryptJSON(external_user_id)
    try {
      let user;
      let userProfiles;
      ...
      if (noOfUsers?.length <= 0) {
        ...
        // create user
        user = await CreateUser({ external_user_id:decrypted_external_user_id });
        // create admin
        ...
        user = await db.UsersModal.findOne({ external_user_id:decrypted_external_user_id });
        if (!user) {
          user = await CreateUser({ external_user_id:decrypted_external_user_id });
      ...
      // fetch profiles
      let systemProfiles = await SystemUsersFunctions.getSystemUsersList({user_id: user._id})
      let organizationProfiles = await OrganizationUsersFunctions.getOrganizationUsersList({user_id: user._id})
      let clientProfiles = await ClientUsersFunctions.getClientUsersList({user_id: user._id})
      userProfiles = {
        systemProfiles,
  ...
  static getProfileConfig = async (req, res) => {
    ...
    try {
      let access = await UserAccessFunctions.FindUserAccessList({role_id: decrypted_external_role_id})
